...
intelItems = ["Land_Laptop_unfolded_F","Land_Suitcase_F","Land_Map_unfolded_F","Land_FilePhotos_F"];
...
FUNC(checkTS) = {
	private["_stillTrue"];
	_stillTrue = _this;
	while {!(call TFAR_fnc_isTeamSpeakPluginEnabled) && _stillTrue} do {
		cutRsc ["TFARTitle","PLAIN"];
		_stillTrue = false;
		//_stillTrue call FUNC(checkTS);
		sleep 4;
	};

	if (call TFAR_fnc_isTeamSpeakPluginEnabled && !_stillTrue) exitWith {
		cutText ["","BLACK IN"];
		_stillTrue = true;
	};
};